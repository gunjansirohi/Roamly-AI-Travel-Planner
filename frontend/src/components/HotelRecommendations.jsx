...
function HotelCard({ hotel, index, favorite, onOpen, onFavorite }) { return <motion.article layout initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(index * .04, .25) }} className="hotel-card" onClick={onOpen}><div className="hotel-card-body"><div className="hotel-card-top"><span className="hotel-category">{hotel.category || "Hotel"}</span><button className="hotel-favorite" onClick={(event) => { event.stopPropagation(); onFavorite(); }} aria-label="Save favorite">{favorite ? "♥" : "♡"}</button></div><div className="hotel-card-title"><h3>{hotel.name}</h3><b>★ {hotel.rating ? hotel.rating.toFixed(1) : "—"}</b></div><p className="hotel-reviews">{priceLabels[hotel.priceLevel]}{hotel.nightlyRate ? ` · ${hotel.currency || "USD"} ${hotel.nightlyRate}/night` : ""}</p><p className="hotel-address">{hotel.address}</p>{hotel.phone && <p className="hotel-contact">Phone: {hotel.phone}</p>}<div className="hotel-card-actions" onClick={(event) => event.stopPropagation()}>{hotel.website && <a href={hotel.website} target="_blank" rel="noreferrer">Website</a>}{hotel.mapsUrl && <a className="outline" href={hotel.mapsUrl} target="_blank" rel="noreferrer">Map</a>}</div></div></motion.article>; }
...
function HotelModal({ hotel, isFavorite, onClose, onFavorite, onAdd }) {
  return <motion.div className="hotel-modal-backdrop" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={onClose}><motion.article className="hotel-modal" initial={{ opacity: 0, y: 25 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 25 }} onMouseDown={(event) => event.stopPropagation()}><button className="hotel-close" onClick={onClose} aria-label="Close hotel details">×</button><div className="hotel-modal-body"><p className="hotel-eyebrow">{hotel.category || "Hotel"} · {priceLabels[hotel.priceLevel]}</p><h2>{hotel.name}</h2><p className="hotel-modal-rating">★ {hotel.rating ? hotel.rating.toFixed(1) : "—"}</p><p>{hotel.address}</p>{hotel.nightlyRate && <p>Budget: {hotel.currency || "USD"} {hotel.nightlyRate} per night</p>}{hotel.phone && <p>Phone: {hotel.phone}</p>}<div className="hotel-actions">{hotel.mapsUrl && <a href={hotel.mapsUrl} target="_blank" rel="noreferrer">View on Google Maps</a>}{hotel.website && <a href={hotel.website} target="_blank" rel="noreferrer" className="outline">Visit Website</a>}<button onClick={onAdd}>Add to Trip</button><button className="outline" onClick={onFavorite}>{isFavorite ? "Saved ♥" : "Save Favorite"}</button></div></div></motion.article></motion.div>;
}
